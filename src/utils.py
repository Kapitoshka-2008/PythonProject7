...
def load_transactions(file_path: str) -> pd.DataFrame:
    ...
    try:
        ...
        # Преобразуем DataFrame в JSON для промежуточной обработки
...
def get_currency_rates(currencies: List[str]) -> List[Dict[str, Any]]:
    ...
    if not CURRENCY_API_KEY:
        ...
        return json.dumps([{"currency": cur, "rate": "Error: API key missing"} for cur in currencies])
    ...
    try:
        ...
        if data.get("result") == "error":
            ...
            return json.dumps([{"currency": cur, "rate": f"API Error: {error_type}"} for cur in currencies])

        rates = data.get("conversion_rates")
        if not rates:
            logger.error("No conversion_rates found in ExchangeRate-API response.")
            return json.dumps([{"currency": cur, "rate": "API Error: No rates data"} for cur in currencies])
        ...
        for currency_code in currencies:
            ...
            if rate is not None:
                ...
                logger.warning(f"Currency {currency_code} not found in ExchangeRate-API response.")
        ...
        return json.dumps([{"currency": cur, "rate": "Network Error"} for cur in currencies])
    except Exception as e:
        logger.error(f"Unexpected error in get_currency_rates: {e}")
        return json.dumps([{"currency": cur, "rate": "Unexpected Error"} for cur in currencies])
...
def get_stock_prices(stocks: List[str]) -> List[Dict[str, Any]]:
    ...
    if not STOCK_API_KEY:
        ...
        return json.dumps([{"stock": stock, "price": "Error: API key missing"} for stock in stocks])
    ...
    for stock_symbol in stocks:
        ...
        try:
            ...
            if "Global Quote" in data and data["Global Quote"] and "05. price" in data["Global Quote"]:
                ...
                logger.warning(f"Unexpected response structure from Alpha Vantage for {stock_symbol}")
