...
def get_currency_rates(currencies: List[str]) -> List[Dict[str, Any]]:
    ...
    if not CURRENCY_API_KEY:
        ...
        return json.dumps(
            [
                {"currency": cur, "rate": "Error: API key missing"}
    ...
    base_currency = "USD"
    url = (
        f"https://v6.exchangerate-api.com/v6/{CURRENCY_API_KEY}/latest/"
        f"{base_currency}"
    )
    ...
    try:
        ...
        if data.get("result") == "error":
            ...
            return json.dumps(
                [
                    {"currency": cur, "rate": f"API Error: {error_type}"}
                    for cur in currencies
                ]
            )
        ...
        if not rates:
            ...
            return json.dumps(
                [
                    {"currency": cur, "rate": "API Error: No rates data"}
                    for cur in currencies
                ]
            )
        ...
        for currency_code in currencies:
            ...
            if rate is not None:
                ...
                logger.warning(
                    f"Currency {currency_code} not found in ExchangeRate-API response."
                )
        ...
        logger.error(
            f"Error getting currency rates from ExchangeRate-API: {e}"
        )
        return json.dumps(
            [{"currency": cur, "rate": "Network Error"} for cur in currencies]
        )
    except Exception as e:
        logger.error(f"Unexpected error in get_currency_rates: {e}")
        return json.dumps(
            [{"currency": cur, "rate": "Unexpected Error"} for cur in currencies]
        )
...
def get_stock_prices(stocks: List[str]) -> List[Dict[str, Any]]:
    ...
    if not STOCK_API_KEY:
        ...
        return json.dumps(
            [
                {"stock": stock, "price": "Error: API key missing"}
                for stock in stocks
            ]
        )
    ...
    for stock_symbol in stocks:
        url = (
            ...
            f"&symbol={stock_symbol}&apikey={STOCK_API_KEY}"
        )
        try:
            response = requests.get(url)
            response.raise_for_status()
            ...
            if (
                "Global Quote" in data and data["Global Quote"] and
                "05. price" in data["Global Quote"]
            ):
                ...
                logger.warning(
                    f"Alpha Vantage API error for {stock_symbol}: "
                    f"{data['Error Message']}"
                )
                result.append({"stock": stock_symbol, "price": "API Error"})
            elif "Note" in data:
                logger.warning(
                    f"Alpha Vantage API Note for {stock_symbol}: "
                    f"{data['Note']}"
                )
                result.append({"stock": stock_symbol, "price": "API Limit/Error"})
            else:
                logger.warning(
                    f"Unexpected response structure from Alpha Vantage for {stock_symbol}"
                )
                result.append({"stock": stock_symbol, "price": "Invalid data"})

        except requests.exceptions.RequestException as e:
            logger.error(
                f"Error getting stock price for {stock_symbol} from Alpha Vantage: {e}"
            )
            result.append({"stock": stock_symbol, "price": "Network Error"})
        except Exception as e:
            logger.error(
                f"Unexpected error processing stock {stock_symbol}: {e}"
            )
